...
struct wumpus_world
{
	...
	wumpus_world( const coordinate & pos, direction dir, const coordinate & wum, const coordinate & gol, const std::set< coordinate > & pit ) :
	...
	typename world::action operator( )( )
	{
		...
		if ( plan.empty( ) )
		{
			A_star( path_finder,
					...
					[&]( const decltype( path_finder ) & pf ) { return possibly_safe.count( pf.first ) != 0 && un_visited.count( pf.first ) != 0; },
					[](const size_t &){}, std::back_inserter( plan ) );
		}
