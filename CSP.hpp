		...
		std::list< local_constraint< VARIABLE_ID_T, VARIABLE_T > > & constraint_set, OUTITER result )
{
	...
	const auto & next_element = * std::min_element(
			...
			{
				return
						( partial_assignment.count( l.first ) == 0 ? l.second.first.size( ) : std::numeric_limits< size_t >::max( ) ) <
		...
		constraint_set.push_back(
				make_local_constraint< VARIABLE_ID_T, VARIABLE_T >(
					...
					[=]( const std::vector< std::reference_wrapper< const VARIABLE_T > > & ass )
					{
						...
						{ if ( std::count( assigned_neighbor_value[i].begin( ), assigned_neighbor_value[i].end( ), ass[i] ) == 0 ) { return true; } }
						return false;
					} ) );
	}
