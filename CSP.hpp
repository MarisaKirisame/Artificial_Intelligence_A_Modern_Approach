...
struct local_constraint
{
	...
	local_constraint( const std::vector< VARIABLE_ID_T > & v, const std::function< bool( std::vector< std::reference_wrapper< const VARIABLE_T > > ) > & f ) :
...
OUTITER backtracking_search(
		std::map< VARIABLE_ID_T, std::pair< std::list< VARIABLE_T >, std::map< VARIABLE_ID_T, std::list< VARIABLE_T > > > > variable,
		...
		std::list< local_constraint< VARIABLE_ID_T, VARIABLE_T > > & constraint_set, OUTITER result )
{
	...
	if (
			std::any_of(
				...
				constraint_set.end( ),
				[&]( const local_constraint< VARIABLE_ID_T, VARIABLE_T > & con ) { return ! con( partial_assignment ); } ) ) { return result; }
							...
							auto generate =
								[&]( const auto & self, std::vector< std::reference_wrapper< const VARIABLE_T > > & arg )
								{
									...
									else
									{
										...
										if ( parameters[ arg.size( ) ].get( ).empty( ) ) { need_var = true; }
	...
	const auto & next_element = * std::min_element(
			variable.begin( ),
			variable.end( ),
			[&]( const auto & l, const auto & r )
			{
			return ( partial_assignment.count( l.first ) == 0 ? l.second.first.size( ) : std::numeric_limits< size_t >::max( ) ) <
			( partial_assignment.count( r.first ) == 0 ? r.second.first.size( ) : std::numeric_limits< size_t >::max( ) );
			} );
	...
	if ( next_element.second.first.empty( ) )
	{
		...
		constraint_set.push_back(
				make_local_constraint< VARIABLE_ID_T, VARIABLE_T >(
					...
					for ( size_t i = 0; i < ass.size( ); ++i ) { if ( std::count( assigned_neighbor_value[i].begin( ), assigned_neighbor_value[i].end( ), ass[i] ) == 0 ) { return true; } }
					return false;
					} ) );
	}
		...
		std::list< local_constraint< VARIABLE_ID_T, VARIABLE_T > > constraint_set, OUTITER result )
{
	...
	constraint_set.remove_if( []( const local_constraint< VARIABLE_ID_T, VARIABLE_T > & t ){ return t.related_var.size( ) < 2; } );
